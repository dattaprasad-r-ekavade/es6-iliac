...
- The fallen Bhagiratha's cache waits in the room they died in. Descending again returns to that
...
- **Iteration 14** — the run. `RunState`, the camp decision, the `N x T` payout curve, the
  summary screen.
- **Iteration 15** — succession. Named Bhagirathas, the fallen cache, half the pack lost, keys
